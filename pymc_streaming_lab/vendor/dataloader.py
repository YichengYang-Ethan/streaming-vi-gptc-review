...
.. code-block:: python
    ...
    import numpy as np
    import pymc as pm
    from pymc_extras.variational.streaming import DataLoader, parquet_source
...
class DataLoader:
    ...
    def __init__(
        ...
        *,
        ...
        # The raw (un-shuffled) source, for a complete deterministic pass over every
        # row (see complete_batches); shuffle only matters for training minibatches.
    ...
    def complete_batches(self) -> Iterator[np.ndarray]:
        """Yield one epoch covering *every* row, including the trailing partial batch
        that :meth:`__iter__` drops (``drop_last=False`` semantics).

        The final batch may have fewer than ``batch_size`` rows, so feed these only
        where a variable-size placeholder is acceptable — e.g. a final exact full-data
        log-density pass — not the fixed-shape training loop. Batches are preprocessed
        and dtype-cast like :meth:`__iter__`, but skip the fixed-``batch_size`` check.
        """
        for batch in _rebatch(
            self._raw_factory(), self._batch_size, self._sample_shape, drop_last=False
        ):
            if self._preprocess_fn is not None:
                batch = self._preprocess_fn(batch)
            yield np.array(batch, dtype=self._dtype)
...
def _rebatch(
    ...
    if not drop_last and have > 0:
        # The trailing partial batch (< batch_size rows) that drop_last discards.
